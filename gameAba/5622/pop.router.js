...
POP.play = function() {
    ...
    if (POP.nextBubble < 0) {
        POP.nextBubble = POP.rnd(10 - POP.level, 50); //改产生的泡泡数量
        bubble_speed = POP.rnd(6 + POP.level) * -1;
    ...
    for (i = 1; i <= POP.MAX_LIVES; i++) {
        ...
        POP.draw.circle( (i * 25) + (POP.W - 280), 20, 10, live_col); //定义命的圆圈位置
    }
    endseconds = ~~(( new Date().getTime() - POP.gameStart ) / 1000);
